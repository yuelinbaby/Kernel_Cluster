...
encorfile=paste0(dir,"/",samid,".encorder.mse.csv")
encor <- read.table(encorfile,sep=" ",header=F)
colnames(encor)= paste0("p",seq(0,9,1))
#encor <- cbind(samid,samfrac,rownames(encor),encor)
#只看k=3的结果
dy=round(diff(as.numeric(encor[1,])),5)
if (length(which(diff(sign(dy)) != 0))==0){
  if (dy[1]>0){sign_change = 0}else{sign_change=8}
}else{
...
if (file.exists(file_path)) {
    seurat_file <- paste0(dir,"/kernel_3_",sign_change,".rds")
    ...
    for(algid in c(1,3:4)){
      algname=algorithms[algid];tag=paste0("optimal_dataset_",algname);
      cluresult=seurat_cluster(seurat,algid,paste0("optimal_dataset_",algname))
      clu=cluresult[[1]];
      seurat[[tag]] <- clu;
    }
    #接下来，统计每种方法得到的聚类结果的指标。
    Method=c("kernel_Louvain","kernel_SLM","kernel_Leiden")
    clusta  <- seurat_ori_cluster_sta_function(seurat,"kernel")
    ...
    seurat_file_rename=paste0(dir,"/",samid,"_kernel_optimized.rds")
    saveRDS(seurat,seurat_file_rename)
    saveRDS(seurat,seurat_file)
    #saveRDS(seurat_smooth[[2]],seurat_file_rename)
    #saveRDS(seurat_smooth[[2]],seurat_file)    
    write.table(clusta,paste0(dir,"/sample_Kernel_cluster.sta"),row.names = T,col.names = T,quote=F,sep="\t")
}
