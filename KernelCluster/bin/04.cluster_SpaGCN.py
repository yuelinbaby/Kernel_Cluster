...
#indir="/media/disk1/SYSU/project/bin/KernelCluster_pipeline_github/test/input"
#outdir="/media/disk1/SYSU/project/bin/KernelCluster_pipeline_github/test/output"
#samid="151673"
indir = args.indir
outdir = args.outdir
samid = args.samid

#因为SpaGCN包安装在3.8，需要制定导入路径
#import sys
#sys.path.append('/home/cfpeng/.local/lib/python3.8/site-packages/')
#import SpaGCN as spg
...
adata=sc.read_10x_h5(f'{indir}/{samid}_filtered_feature_bc_matrix.h5')
...
adata.write_h5ad(f'{outdir}/sample_data.h5ad')
#Read in hitology image
#img=cv2.imread(f'{outdir}/{samid}/{samid}_map.jpg')
img=cv2.imread(f'{indir}/{samid}_full_image.tif')
...
cv2.imwrite(f'{outdir}/{samid}_map.jpg', img_new)
...
np.savetxt(f'{outdir}/adj.csv', adj, delimiter=',')
...
#Save results
adata.write_h5ad(f'{outdir}/results.h5ad')
adata.obs.to_csv(f'{outdir}/SpaGCN.result')
